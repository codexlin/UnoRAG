...
def test_eval_cases_file_has_smoke_size() -> None:
	...
	assert any(
		c.kind == "retrieval" and (c.expect.max_rank is not None) for c in cases
	), "expected some retrieval cases to assert max_rank"
	assert any(c.kind == "ingest_http" and "negative" in c.tags for c in cases)
