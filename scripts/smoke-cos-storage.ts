...
try {
	...
	uploaded = true;
	if (stored.contentHash !== expectedHash || stored.sizeBytes !== payload.length) {
	...
	const chunks: Buffer[] = [];
	for await (const chunk of storage.openStream(key)) chunks.push(Buffer.from(chunk));
	if (!Buffer.concat(chunks).equals(payload)) {
		throw new Error("COS streaming download mismatch");
	}
	if (!(await storage.delete(key))) throw new Error("COS DeleteObject did not run");
	uploaded = false;
	if (await storage.exists(key)) throw new Error("COS object remains after delete");
