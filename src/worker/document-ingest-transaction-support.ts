...
type LibraryIdentityRow = { library_id: string };
type DocumentRow = {
...
type VersionRow = { version_status: string };
type JobRow = {
...
type AclRow = { subject_type: string; subject_id: string };
...
export abstract class DocumentIngestTransactionSupport {
	constructor(protected readonly pool: Pool) {}

	protected async lockContext(
		client: PoolClient,
		...
		const aclRows = await client.query<AclRow>(
			...
			WHERE document_id = $1 AND permission = 'read'
	...
	protected isCancellation(context: IngestContext): boolean {
		return (
	...
	protected assertMutableDocument(context: IngestContext): void {
	...
	protected async cancelPendingCleanup(
		client: PoolClient,
		input: DocumentIngestJob,
		libraryId: string,
	): Promise<void> {
	...
	protected async queueSupersededGeneration(
		client: PoolClient,
		input: DocumentIngestJob,
		libraryId: string,
		versionId: string,
	...
	protected async queueFailedStagingGeneration(
		client: PoolClient,
		input: DocumentIngestJob,
		libraryId: string,
	): Promise<void> {
	...
	protected async refreshDocumentFailure(
		client: PoolClient,
		input: DocumentIngestJob,
		libraryId: string,
	): Promise<void> {
	...
	protected async refreshLibrary(
		client: PoolClient,
		input: DocumentIngestJob,
		libraryId: string,
	): Promise<void> {
