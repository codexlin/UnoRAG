...
| [`release-images.yml`](./release-images.yml) | Manual/tag build of three image targets; one build per target is pushed to ACR + GHCR and emitted as regional digest manifests. Real push requires ACR secrets and `dry_run=false`. |
