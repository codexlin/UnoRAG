...
当前源码已采用 Apache-2.0，适合内部预发布与受控试点；在素材权属、完整第三方通知、镜像签名和
目标客户环境复验完成前，不应宣称已完成公开发行，也不应把历史 RC 结论扩大为通用生产认证。
...
2. 生成完整第三方 NOTICE/许可证包，完成 libvips/字体再分发审阅；镜像 SBOM/provenance 已接入但尚未在公开 tag 复验。
3. 接入镜像签名，决定公开完整历史还是经审阅的干净初始历史。
4. 配置公开仓库安全报告、分支保护和维护渠道。
...
1. **发布合规批次**：确认素材/fixture 权属、完整第三方 NOTICE、签名与公开历史策略。
