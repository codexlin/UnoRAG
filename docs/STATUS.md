...
当前适合内部预发布与受控试点；在许可证、素材权属、供应链证明和目标客户环境复验完成前，不应宣称为
已经开放许可的开源版本，也不应把历史 RC 结论扩大为通用生产认证。
...
1. 确认版权主体和最终许可证，提交 `LICENSE`；Apache-2.0 目前只是候选。
2. 完成截图和 `testdata/` fixture 的来源/再分发确认，补齐 `ASSETS.md`；当前 Uno 图形也仍是临时品牌资产。
3. 为源码和四个镜像生成 SBOM、第三方 NOTICE、签名与 provenance，并完成 libvips/字体再分发审阅。
4. 决定公开完整历史还是经审阅的干净初始历史，配置公开仓库安全报告、分支保护和维护渠道。
5. 完成 `UnoRAG` / `Unobyte` 名称与图形在目标地区和软件类别的商标检索。
...
当前最有杠杆的工作不是继续增加 RAG 路径，而是完成第一个可公开、可复验的版本：

1. **发布合规批次**：确认许可证、素材/fixture 权属、NOTICE、SBOM、签名与公开历史策略。
2. **品牌与对外材料批次**：确定正式名称和 Logo，更新产品截图，避免用临时资产发布首个公开版本。
3. **精确 RC 批次**：对同一个 commit 构建不可变镜像，重跑 CI、真实文件、浏览器隔离、升级/回滚和恢复。
4. **公开 v0.1.0**：发布源码、镜像 digest、SBOM/provenance 和版本绑定验收报告。
