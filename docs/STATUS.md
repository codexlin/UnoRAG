...
- `v0.2.2` 加固原生告警去抖与恢复、Redis 分布式限流、Session 即时撤销和安全响应头，并完成默认/高级
  配置分层、旧运行时清理与大模块职责拆分。正式发布结论以本版本绑定的香港环境验收报告为准。
...
`v0.2.2` 已完成代码与文档收敛，正在执行不可变镜像、香港环境升级、公网 smoke 与完整真实文件门禁。历史 PASS
