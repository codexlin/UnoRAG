...
2. 填好 `deploy/config/runtime.env` + `runtime.secret` + `bootstrap.env`（真实密钥与模型 endpoint；勿提交）。
...
| 演练 | 步骤 | 期望 |
|---|---|---|
| Worker drain | `source scripts/compose-env.sh && mk_compose stop lifecycle-worker` → 再 `mk_compose up -d` | 当前步骤结束后退出；lease 可恢复 |
