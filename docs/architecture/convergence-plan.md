...
> **当前**：Eval 无行为变化拆分 **已完成**；AskGraph 提交 1–5（characterization → state/messages/stubs → persistence/lifecycle → AskGraphContext → 分组搬 `nodes/`）**已完成**；下一步提交 6（`topology.py`）。
...
> Eval 拆分已合入；AskGraph 仍为**独立 PR**。提交 1–5 已完成；下一步 **提交 6：`topology.py`**。
...
apps/api/app/graph/          # 提交 2–5 平铺 + nodes/；后续可迁入 ask/
  ...
  ask_graph.py               # facade（re-export）+ topology 组装 + service（待拆）
apps/api/app/graph/ask/      # 目标结构（提交 6+ 可迁入）
...
**AskGraph 当前进度**：提交 1–5 已完成 → **下一刀：提交 6（`topology.py`）**。
...
2. **进行中**：~~Eval 拆分~~ **已完成**；AskGraph（~~提交 1–5~~；下一步 **提交 6 topology**）
