...
> **当前**：Eval 无行为变化拆分 **已完成**；AskGraph 提交 1–4（characterization → state/messages/stubs → persistence/lifecycle → AskGraphContext）**已完成**；下一步提交 5（分组搬 `nodes/`）。
...
> Eval 拆分已合入；AskGraph 仍为**独立 PR**。提交 1–4 已完成；下一步 **提交 5：分组搬 nodes**。
...
```text
apps/api/app/graph/          # 提交 2–4 先落平铺；后续可迁入 ask/
...
apps/api/app/graph/ask/      # 目标结构（提交 5+ 可迁入）
...
**AskGraph 当前进度**：提交 1–4 已完成 → **下一刀：提交 5（分组搬 nodes）**。
...
2. **进行中**：~~Eval 拆分~~ **已完成**；AskGraph（~~提交 1–4~~；下一步 **提交 5 分组搬 nodes**）
