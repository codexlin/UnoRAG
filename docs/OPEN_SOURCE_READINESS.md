...
> 审计日期：2026-08-23
>
> 审计对象：公开仓库当前树、完整 Git 历史、稳定镜像与 GitHub 安全设置
>
> 当前结论：**仓库已公开并采用 Apache-2.0；素材、许可证、SBOM/provenance、漏洞扫描和镜像签名均已在稳定 `v0.1.0` 精确版本验证并发布。**
...
| 镜像 SBOM / provenance | PASS | v0.1.0 四个 GHCR digest 由稳定流水线生成 BuildKit SPDX SBOM 与 SLSA provenance |
| 四镜像构建 | PASS | web、worker、ops、migrator 均按 `linux/amd64` 构建；运行层以 UID 10001 启动并可读取 `LICENSE`/`NOTICE` |
| 镜像签名 | PASS | v0.1.0 四个 GHCR 与 ACR digest 均由 release workflow 使用 GitHub OIDC keyless Cosign 签名并在发布流水线和 HK 升级前 fail-closed 自验 |
...
1. 每个后续正式标签重新验证 attestation、许可证包和签名，不能把 v0.1.0 的结果自动继承给新 digest；
...
`v0.1.0` 已从最终 `main` 提交创建；稳定流水线重新构建四镜像并完成漏洞、SBOM/provenance、Cosign
身份与发布 manifest 门禁。GitHub Release 包含 GHCR/ACR digest manifest、校验和、release notes 与已知
限制；UnoRAG-HK 线上健康端点与稳定 manifest 一致。产品、隔离、恢复、回滚和容量证据见
