...
| 拓扑 | Compose `meriknow-webch` @ `/opt/meriknow` |
...
| ID | 检查项 | 结果 | 证据 |
|----|--------|------|------|
| D1 | 镜像 linux/amd64 构建并 `docker load` | PASS | `meriknow-{web,web-migrator,api}:fa49d86`（后续 API 热修 tag `fa49d86-302cn`） |
...
| D8 | `pilot-smoke`（HTTPS 真实上传→Ask→隔离→替换→删除） | PASS | `/tmp/meriknow-webch-pilot-smoke.log` |
...
| 2 | 同机 openclaw :8443 等 | **接受** — MeriKnow 独占 80/443 |
| 3 | 告警 webhook **暂缓** | **接受** — 待正式飞书后再接 `ops/min_alerts` |
...
```cron
# /etc/cron.d/meriknow-backup 或 root crontab
0 3 * * * cd /opt/meriknow/deploy/compose && ./scripts/backup.sh ./backups/meriknow-$(date +\%Y\%m\%d) >> /var/log/meriknow-backup.log 2>&1
```

- 保留建议：本地 ≥7 天；负责人 codexlin  
- 恢复：`CONFIRM=YES ./scripts/restore.sh ./backups/meriknow-YYYYMMDD`

## 访问

- URL：https://webch.cn  
- Admin：`codexlin@webch.cn`（密码仅存服务器 `bootstrap.env` / 本机 `/tmp/meriknow-webch-admin-pass.txt`，**勿入库**）

## 运维命令

```bash
ssh dusty-ali
cd /opt/meriknow/deploy/compose
